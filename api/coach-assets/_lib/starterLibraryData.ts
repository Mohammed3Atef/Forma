 ...
 * food-groups, and workout-TEMPLATE blueprints — verbatim data, ported so the
 * `POST /api/coach-assets/seed-starter-library` route doesn't depend on `src/`
 * (Vercel's per-function bundler can't resolve the `@/*` alias). Keep in sync
 * with `src/lib/starterLibrary.ts` by hand if that file's data ever changes.
 */
...
function fd(slug: string, en: string, ar: string, quantity: string, calories: number, protein: number, carbs: number, fats: number, category: string): StarterFood {
...
// time the seed route fills the day by drawing exercises round-robin across
