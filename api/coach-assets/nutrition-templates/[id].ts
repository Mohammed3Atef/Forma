import type { VercelRequest, VercelResponse } from '@vercel/node';
import { requireOwningCoach, requireReadAccess } from '../_lib/access';
import { coachNutritionTemplatesCol } from '../_lib/db';
import { NutritionTemplateBodyPatchSchema } from '../_lib/schemas';
import type { CoachNutritionTemplateDoc } from '../_lib/types';
import { HttpError, handleError, methodGuard } from '../../_lib/http';
import { requireUser } from '../../_lib/withAuth';

/** GET/PATCH/DELETE /api/coach-assets/nutrition-templates/:id */

function toPublic(doc: CoachNutritionTemplateDoc) {
  const { _id, coachId, ...fields } = doc;
  return { id: _id, coachId, ...fields };
}

export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    const id = req.query.id as string;
    const col = await coachNutritionTemplatesCol();

    if (req.method === 'GET') {
      const user = await requireUser(req);
      const doc = await col.findOne({ _id: id });
      if (!doc) throw new HttpError(404, 'Nutrition template not found');
      requireReadAccess(user, doc.coachId);
      res.status(200).json(toPublic(doc));
      return;
    }

    if (req.method === 'PATCH') {
      const user = await requireOwningCoach(req);
      const patch = NutritionTemplateBodyPatchSchema.parse(req.body);
      const existing = await col.findOne({ _id: id, coachId: user.id });
      if (!existing) throw new HttpError(404, 'Nutrition template not found');
      const updated: CoachNutritionTemplateDoc = { ...existing, ...patch, updatedAt: Date.now() };
      await col.replaceOne({ _id: id, coachId: user.id }, updated);
      res.status(200).json(toPublic(updated));
      return;
    }

    methodGuard(req, 'DELETE');
    const user = await requireOwningCoach(req);
    const result = await col.deleteOne({ _id: id, coachId: user.id });
    if (result.deletedCount === 0) throw new HttpError(404, 'Nutrition template not found');
    res.status(204).end();
  } catch (e) {
    handleError(res, e);
  }
}
