 ...
 * singleton at Firestore's `clientData/{clientId}/plan/nutrition`. `plan` is
 * in `isCoachOwnedColl`: client read-only, coach/admin(clients.writeAll) write.
 */
const Body = z
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    const col = await clientNutritionPlansCol();
