...
import { clientWorkoutPlansCol } from './_lib/db.js';

/**
 * Port of `planApi.getClientWorkoutPlan/saveClientWorkoutPlan` — the coach-
 * authored singleton at Firestore's `clientData/{clientId}/plan/workout`.
 * `plan` is in `isCoachOwnedColl`: client is read-only; only the assigned
 * coach / admin(clients.writeAll) may write. The nested day/exercise
 * structure is frontend-authored and stored opaquely beyond `id`/`name`.
 */
const Body = z
