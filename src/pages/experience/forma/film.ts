...
POSES.GRID = (ids) => {
  ...
  for (let i = 0; i < 8; i++) {
    const col = i % 4,
      row = i < 4 ? 0 : 1;
    set(T, 'plate' + i, [-1.1 + col * 0.73, 0.55 - row * 0.72, -0.3], [0, Math.PI / 2, 0], [0.1, 1.0, 1.0]);
  }
...
POSES.CHARTS = (ids) => {
  ...
  for (let i = 0; i < 24; i++) {
    const k = i % 5,
      lane = i < 10 ? 0 : 1;
    const v0 = [0.62, 0.44, 0.3, 0.16, 0.04],
      v1 = [0.1, 0.2, 0.26, 0.4, 0.52];
...
export const CHAPTERS = [
  {
    ...
    label: 'Hero',
    vh: 160,
    poses: ['DUMBBELL', 'DUMBBELL'],
    ease: 'mech',
    shift: 0.17,
    cam: [cam([0.28, 0.3, 1.72], [0, 0, 0], 42), cam([-0.95, 0.34, 1.18], [0, 0, 0], 46, 0.02), cam([0.6, 0.09, 0.54], [0.33, 0, 0], 58)],
  },
  {
    id: 'problem',
    label: 'The problem',
    vh: 200,
    poses: ['DUMBBELL', 'EXPLODED', 'BARBELL'],
    ease: 'mech',
    shift: 0.14,
    cam: [cam([0.6, 0.09, 0.54], [0.33, 0, 0], 58), cam([0.3, 0.26, 1.95], [0, 0, 0], 50, -0.03), cam([0, 0.2, 3.35], [0, 0, 0], 40, -0.05)],
  },
  {
    id: 'assembly',
    label: 'Assembly',
    vh: 280,
    poses: ['BARBELL', 'BENCH', 'RACK', 'TRAINER'],
    ease: 'mech',
    shift: 0.04,
    cam: [
  ...
  },
  {
    ...
    vh: 220,
    poses: ['TRAINER', 'TRAINER', 'GRID'],
    ease: 'digi',
    shift: 0.06,
    cam: [cam([0.55, 1.25, 4.7], [0, 1.05, -0.1], 38, 0), cam([0.05, 1.15, 1.1], [0, 1.1, -0.3], 44, 0), cam([0, 0.05, 3.15], [0, 0.02, -0.3], 45, 0)],
  },
  {
    id: 'clients',
  ...
  },
  {
    ...
    label: 'The mark',
    vh: 160,
    poses: ['CONVERGE', 'COLLAPSE', 'COLLAPSE'],
    ease: 'digi',
    cam: [cam([0, 0.03, 3.4], [0, 0, -0.32], 44), cam([0, 0, 2.6], [0, 0, -0.2], 46), cam([0, 0, 2.35], [0, 0, 0], 48)],
  },
  {
    id: 'close',
    label: 'Close',
    vh: 100,
    poses: ['COLLAPSE', 'COLLAPSE'],
    ease: 'digi',
    cam: [cam([0, 0, 2.35], [0, 0, 0], 48), cam([0, 0, 2.35], [0, 0, 0], 48)],
    static: true,
  },
];
