...
/* ============================================================
   ...
   aside from the mark texture path (moved under /experience/).
   ============================================================ */
import * as THREE from 'three';
import { knurlNormal, grainNormal, strandNormal } from './stage';
...
export function materials() {
  ...
  const M = {
    ...
    alu: new THREE.MeshStandardMaterial({ name: 'machined_alu', color: 0xb4b4b8, metalness: 1, roughness: 0.22 }),
    chrome: new THREE.MeshStandardMaterial({ name: 'chrome', color: 0xe8e8ea, metalness: 1, roughness: 0.06 }),
    ...
    cable: new THREE.MeshStandardMaterial({
      ...
      emissive: new THREE.Color(0xff8b02),
      emissiveIntensity: 0,
    }),
    accent: new THREE.MeshStandardMaterial({
      name: 'anodized_accent',
      color: 0xff8b02,
      metalness: 0.8,
      roughness: 0.3,
      emissive: new THREE.Color(0xff6a02),
      emissiveIntensity: 0,
    }),
  };
...
export function buildParts(scene) {
  ...
  const shaft = new THREE.Mesh(new THREE.CylinderGeometry(0.0145, 0.0145, 1, 40, 1), M.knurled);
  shaft.geometry.rotateZ(Math.PI / 2);
  reg('shaft', shaft, 'shaft');

  /* two sleeves that ride the shaft ends */
  for (let i = 0; i < 2; i++) {
    const s = new THREE.Mesh(new THREE.CylinderGeometry(0.024, 0.024, 0.3, 32, 1), M.chrome);
  ...
  for (let i = 0; i < 2; i++) {
    ...
    const pin = new THREE.Mesh(new THREE.CylinderGeometry(0.0075, 0.0075, 0.05, 16), M.accent);
    pin.name = 'pin';
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const h = new THREE.Mesh(hoG, M.coat);
    h.name = 'housing';
    const s = new THREE.Mesh(shG, M.chrome);
    s.name = 'sheave';
  ...
  const markTex = new THREE.TextureLoader().load('/experience/forma-mark.png');
  ...
  const fPlane = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({ map: markTex, transparent: true, opacity: 0, depthWrite: false, toneMapped: false }),
  );
