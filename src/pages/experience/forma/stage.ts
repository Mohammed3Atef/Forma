...
/* ============================================================
   ...
   Ported from the Forma.html design (claude.ai/design) as close to
   verbatim as possible — this is a self-contained vanilla three.js
   "director" engine, not idiomatic React/R3F code, so it's kept as
   plain JS logic (ts-nocheck) rather than force-fit into typed React
   patterns that would risk changing its (carefully hand-tuned) behavior.
   ============================================================ */
...
export function knurlNormal(rep = 26) {
  return canvasTex(
    ...
    (g, w, h) => {
      ...
      line(Math.PI / 4, '#b0a0ff');
      line(-Math.PI / 4, '#5060ff');
    },
    rep,
    rep,
  );
  ...
  return canvasTex(
    ...
    (g, w, h) => {
      ...
      for (let i = -h; i < h * 2; i += 9) {
        g.strokeStyle = '#a898ff';
        g.beginPath();
        g.moveTo(0, i);
        g.lineTo(w, i + 26);
        g.stroke();
        g.strokeStyle = '#6070ff';
        g.beginPath();
        g.moveTo(0, i + 4);
        g.lineTo(w, i + 30);
        g.stroke();
...
function studioEnvScene() {
  ...
  const shell = new THREE.Mesh(new THREE.SphereGeometry(40, 16, 12), new THREE.MeshBasicMaterial({ color: 0x060606, side: THREE.BackSide }));
  s.add(shell);
  return s;
}
...
export class CameraRig {
  ...
  set(a, b, k) {
    const lerp3 = (out, A, B) => out.set(A[0] + (B[0] - A[0]) * k, A[1] + (B[1] - A[1]) * k, A[2] + (B[2] - A[2]) * k);
