...
export function MyAssessment() {
  ...
  const q = useQuery({ queryKey: ['assessment', uid], queryFn: () => fetchMyAssessment(uid), enabled });
  const a = q.data ?? null;
  const status = assessmentStatus(a);

  if (editing && a) {
    return <AssessmentWizard uid={uid} displayName={displayName} initial={a} onDone={() => { setEditing(false); void q.refetch(); }} />;
  }
  ...
  return (
    <div className="anim-rise space-y-4">
      <TopBar
        ...
        right={<span className={`chip ${PILL[status]}`}>{t(`assessment.status.${status}`)}</span>}
      />

      {q.isLoading ? (
        <p className="py-8 text-center text-sm text-earth-muted">{t('auth.working')}</p>
      ) : !a ? (
        <p className="py-8 text-center text-sm text-earth-muted">{t('assessment.notCompleted')}</p>
      ) : (
        <>
          <div className="card flex items-center justify-between text-[13px]">
            <span className="text-earth-muted">{t('assessment.lastUpdated')}</span>
            <span className="font-mono">{a.updatedAt ? new Date(a.updatedAt).toLocaleDateString(i18n.language) : '—'}</span>
          </div>
          {(status === 'reviewed' || status === 'updated_after_review') && a.reviewedAt && (
            <div className="card flex items-center justify-between text-[13px]">
              <span className="text-earth-muted">{t('assessment.reviewedOn')}</span>
              <span className="font-mono">{new Date(a.reviewedAt).toLocaleDateString(i18n.language)}</span>
            </div>
          )}
          {a.coachNotes?.trim() && (
          ...
          <button type="button" data-testid="assessment-edit" className="btn-primary w-full" onClick={() => setEditing(true)}>
