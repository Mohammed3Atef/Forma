...
export function WeekStrip({
  ...
  onSelect?: (key: string, done: boolean) => void;
}) {
  ...
  return (
    <div className="-mx-4 flex gap-1.5 overflow-x-auto px-4 py-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
      {days.map((d) => {
        ...
        return (
          ...
          >
            <p
              className={`font-mono text-[9px] uppercase tracking-[0.07em] ${isToday ? "text-brand-ink/70" : "text-earth-subtle"}`}
            >
              {label}
            </p>
            <p
              className={`num text-[15px] ${isToday ? "text-brand-ink" : d.done ? "text-success" : "text-earth-muted"}`}
            >
              {date.getDate()}
            </p>
          </button>
