...
export function TopBar({ title, eyebrow, sub, right, onBack, onTitleClick, sticky, testId }: TopBarProps) {
  const { i18n } = useTranslation();
  const rtl = i18n.dir() === 'rtl';
  // `.h1` uses a very tight line-height; with `truncate` (overflow-hidden) that
  // clips glyph tops/bottoms — worse in RTL (deep descenders). Give the
  // truncated title a taller line-box so nothing is cut.
  const titleClass = `h1 truncate ${rtl ? 'leading-[1.45]' : 'leading-[1.3]'}`;
  return (
    <div
