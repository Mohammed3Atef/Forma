 ...
 */
test('assessment renders correctly, or is completed through the real wizard if missing', async ({ page }) => {
  ...
  if (!isNotCompleted) {
    ...
    console.log('[assessment.spec] assessment already completed — verified read-only render, did not resubmit.');
    return;
  }
  ...
  await expect(page.getByTestId('client-home')).toBeVisible({ timeout: 15_000 });
});
