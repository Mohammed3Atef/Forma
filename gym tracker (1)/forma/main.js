/* ============================================================
   FORMA — main: the director.
   scroll → progress → chapter + t → poses · camera · materials · UI
   ============================================================ */
import * as THREE from "three";
import { createStage, CameraRig } from "./stage.js";
import { buildParts } from "./parts.js";
import { CHAPTERS, Film, POSES, applyTargets } from "./film.js";
...
const ui = mountUI(document.getElementById("ui"));

const IDS = kit.list.filter((p) => p.kind !== "cable").map((p) => p.id);
const REDUCED = window.matchMedia("(prefers-reduced-motion: reduce)").matches;

/* make every material fadeable */
Object.values(kit.M).forEach((m) => { m.transparent = true; m.opacity = 1; });

/* ---------- scroll spine ---------- */
document.getElementById("spacer").style.height = film.total + "vh";
...
/* ---------- copy deck ---------- */
const beats = [...document.querySelectorAll("[data-ch]")].map((el) => ({
  el, ch: el.dataset.ch,
...
function structLevel(id, t) {
  switch (id) {
    case "hero": case "problem": case "assembly": return 1;
    case "hinge": return 1 - win(t, 0.68, 1) * 0.82;
    case "realtime": return 1;
    case "handoff": return 0.3 + win(t, 0.4, 1) * 0.6;
    case "progress": return 1 - win(t, 0.34, 0.8) * 0.7;
    case "final": case "close": return 1;
    case "scale": return 0.55;
    default: return 0.16;
  }
}
...
function matState(id, t) {
  ...
  switch (id) {
    case "hero": s.accent = win(t, 0.55, 1) * 0.6; break;
    case "problem": s.accent = 0.6; break;
    case "assembly": s.accent = 0.4; break;
    case "hinge": {
      const ig = win(t, 0.02, 0.3), di = win(t, 0.3, 0.56), re = win(t, 0.56, 1);
    ...
    case "close": s.mesh = 0; s.edge = 0; s.node = 0; s.fmark = 0; s.accent = 0; s.light = 0.3; s.contact = 0; s.stream = 0; break;
    case "final": {
      const up = win(t, 0, 0.34), out = win(t, 0.5, 0.86), fm = win(t, 0.36, 0.54);
      s.mesh = 0.06; s.edge = Math.max(0.32 + up * 0.68 - out, 0);
      s.node = 1 - out; s.fmark = fm; s.accent = 1 + fm * 1.4;
      s.light = 0.3 + fm * 0.5; s.contact = 0;
      s.stream = Math.max(0, 0.85 - out); s.streamGlow = 1.6;
      break;
    }
    default:
      s.mesh = 0.06; s.edge = 0.52; s.node = 0.95; s.accent = 1.1; s.light = 0.3; s.contact = 0;
      s.stream = 0.92; s.streamGlow = 1.35;
  }
  return s;
}
...
function contactFor(id, t) {
  switch (id) {
    case "hero": return { y: -0.3, s: 1.5 };
    case "problem": return { y: -0.3, s: lerp(1.5, 2.9, t) };
    case "assembly": return { y: 0.005, s: lerp(2.9, 4.6, t) };
    case "hinge": return { y: 0.005, s: 4.6 };
    default: return { y: 0.005, s: 4.6 };
  }
}
...
const PANEL_W = {
  hinge: 960, clients: 960, builder: 880, library: 600, handoff: 960,
  progress: 880, business: 880, scale: 820, beforeafter: 960,
};
const GUTTER = 44;
/* Right edge of the copy column, derived from the CSS rather than measured
   (measuring is circular once body.stack changes the column's width). */
...
const FRAGS = [
  ...
  ["fsheet", 0.40, -0.06, -430, 6, -16],
  ["fpdf", 0.09, 0.31, -560, -5, 12],
  ["fphotos", 0.42, 0.30, -300, -7, -12],
...
function placeUI(id, t) {
  ...
  switch (id) {
    ...
    case "clients": {
      ...
      // the table recedes so one extracted client reads clearly
    ...
    case "builder": {
      const p1 = band(t, 0, 0.37, 0.16), p2 = band(t, 0.35, 0.7, 0.16), p3 = band(t, 0.68, 1, 0.16);
    ...
    case "library": {
      const a = band(t, -0.15, 0.54, 0.16), b = band(t, 0.5, 1.35, 0.16);
    ...
    case "handoff": {
      ...
      const c = win(t, 0.14, 0.5);       // card travels + rotates
      ui.set("cc0", { o: band(t, 0.1, 0.54, 0.18), x: lerp(vx(0.0), vx(0.06), c), y: lerp(-12, 0, c), z: lerp(150, 200, c), ry: lerp(-4, 88, c), s: lerp(1.02, 1.14, c) });
      const ph = win(t, 0.5, 0.88);        // phone continues the rotation
      ui.set("phone", { o: band(t, 0.48, 1.7, 0.14), x: lerp(vx(0.06), vx(0.05), ph), y: -18, z: lerp(200, 120, ph), ry: lerp(-86, 0, ph), s: lerp(0.98, 0.9, ph) });
      break;
    }
    case "realtime": {
    ...
    case "beforeafter": {
      ...
      FRAGS.forEach(([k, fx, fy, z, rx, ry], i) => {
        ...
        const d = 1 - conv;
        ui.set(k, {
          o: scatter, x: lerp(sx, -20 + i * 10, conv), y: lerp(sy, -10 + i * 6, conv),
          z: lerp(z, -60, conv), rx: rx * d, ry: ry * d, s: lerp(1, 0.72, conv),
        });
      });
...
const _m = new THREE.Matrix4(), _s = new THREE.Vector3(1, 1, 1), _qq = new THREE.Quaternion();
...
function step(dt, forceProgress) {
  if (forceProgress != null) { target = forceProgress; progress = forceProgress; }
  ...
  const A = film.poseFor(pp.a, IDS), B = film.poseFor(pp.b, IDS);
  const ms = matState(ch.id, t);
  const sL = structLevel(ch.id, raw);
  ms.edge *= sL; ms.node *= sL; ms.streamGlow *= Math.max(0.35, sL);
  if (ch.id !== "hero" && ch.id !== "problem" && ch.id !== "assembly" && ch.id !== "hinge") ms.stream *= sL;
  applyTargets(kit, A, B, pp.k, ms);

  // materials
  Object.values(kit.M).forEach((m) => { if (m !== kit.M.cable) m.opacity = ms.mesh; });
  ...
  if (kit.fMark.visible) {
    // born at the centre of the collapse ring, then grown out of it
    ...
    kit.fMark.position.set(
      0,
      lerp(0, 0.64, em) + Math.sin(envRot * 1.4) * 0.006 * em,
      lerp(-0.2, 0, em)
    );
  ...
  // fit must consider BOTH axes (the phone and library wall are tall) and
  // positive-z panels are further magnified by the container's perspective.
  const vw = window.innerWidth, vh = window.innerHeight;
  const pw = PANEL_W[ch.id] || 0;
  const wide = vw >= 900;
  let fit = null, uiShift = 0, uiLift = 0, stack = false;

  if (wide && pw > 0) {
    const need = copyRightPx(vw) + GUTTER;
    // largest scale that leaves room for the copy AND keeps the panel onscreen
    const fitCap = (vw - 14 - need) / pw;
    if (fitCap >= SIDE_BY_SIDE_MIN_FIT) {
      fit = Math.min(1, (vw - 120) / 1010, (vh - 96) / 700, fitCap);
      const half = (pw * fit) / 2;
      // this is the constraint that was missing: clear the type column
  ...
  if (stack) {
    // copy bottom-anchored over a scrim; the panel is fitted into the band
    // between the header and the copy, then centred within it.
    const bandTop = HEADER_H;
    const bandBottom = vh - vh * 0.08 - COMPACT_COPY_H - 14;
    const band = Math.max(170, bandBottom - bandTop);
    fit = Math.min(1, (vw - 48) / 1010, band / 640);
  ...
  document.body.classList.toggle("stack", stack);
  uiShiftPx = uiShift;
  stage.setShift(uiShift / vw);
  document.getElementById("ui").style.transform =
    `translate(${uiShift.toFixed(1)}px,${uiLift.toFixed(1)}px) scale(${fit.toFixed(3)})`;

  // idle life: slow environment rotation so a still frame still breathes
  envRot += dt * 0.02;
  stage.scene.environmentRotation && stage.scene.environmentRotation.set(0, envRot, 0);
  ...
  const cl2 = document.getElementById("chlabel");
  const want = `<b>${String(index + 1).padStart(2, "0")}</b> ${ch.label}`;
  if (cl2.innerHTML !== want) cl2.innerHTML = want;
...
window.__forma = {
  step,
  seek(p) { step(0.016, Math.min(1, Math.max(0, p))); },
  at(i, t = 0.5) {
...
const tick = setInterval(() => {
  ...
  document.getElementById("pct").textContent = String(Math.floor(pct)).padStart(3, "0");
  if (pct >= 100) {
    clearInterval(tick);
