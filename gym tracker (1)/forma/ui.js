...
function fragSheet() {
  const rows = [["W1", "Bench", "80×8", "80×8", "80×6"], ["W2", "Bench", "82.5×8", "82.5×7", "80×8"],
  ["W3", "Bench", "82.5×8", "85×6", "82.5×7"], ["W4", "Bench", "85×8", "85×7", "85×6"],
  ["W1", "Squat", "110×6", "110×6", "110×5"], ["W2", "Squat", "112×6", "112×6", "110×6"]];
...
function dashboard() {
  ...
  <aside class="app-nav"><div class="app-brand"><i></i></div>
    <nav>${["Overview", "Clients", "Programs", "Library", "Messages", "Business"]
      .map((n, i) => `<a class="${i === 1 ? "on" : ""}">${n}</a>`).join("")}</nav>
  ...
  <main class="app-main">
    ...
    <div class="kpi-row">
      ${[["Active clients", "38"], ["Renewals this week", "6"], ["Adherence", "88%"], ["Unread", "14"]]
    ...
    ${CLIENTS.map(([n, av, st, ad, w, as, ms, rn]) => `<tr>
      ...
      <td class="num">${ms || "—"}</td><td class="num">${rn}</td></tr>`).join("")}
    </tbody></table>
  </main></div>`;
}
...
function builderWorkout() {
  const rows = [["Warm-up", "Bike + shoulder prep", "—", "6 min", "—", "—"],
  ...
  return `<div class="app bld"><header class="app-top"><div><div class="app-eye">PROGRAM · PHASE II</div>
    ...
    <tbody>${rows.map(([a, b, c, d, e, f]) => `<tr><td class="mono dim">${a}</td><td>${b}</td>
      <td class="num">${c}</td><td class="num">${d}</td><td class="num">${e}</td><td class="num">${f}</td>
      <td><span class="vid"></span></td></tr>`).join("")}</tbody></table></div>`;
}
function builderNutrition() {
  const meals = [["Breakfast", "Oats · Whey · Blueberries", 512, 38, 62, 11],
  ...
  return `<div class="app bld"><header class="app-top"><div><div class="app-eye">NUTRITION</div>
    ...
    <div class="macro">${[["PROTEIN", "158 g"], ["CARBS", "195 g"], ["FAT", "66 g"], ["FIBRE", "31 g"]]
      .map(([l, v]) => `<div><span>${l}</span><b>${v}</b></div>`).join("")}</div>
    <table class="tbl"><thead><tr><th>Meal</th><th>Foods</th><th>kcal</th><th>P</th><th>C</th><th>F</th></tr></thead>
    <tbody>${meals.map(([m, f, k, p, c, ft]) => `<tr><td>${m}</td><td class="dim">${f}</td>
      <td class="num">${k}</td><td class="num">${p}</td><td class="num">${c}</td><td class="num">${ft}</td></tr>`).join("")}</tbody></table></div>`;
}
function builderCardio() {
  const rows = [["Mon", "Zone 2 walk", "40 min", "Z2 · 60-70%"], ["Wed", "Intervals", "18 min", "8×30s hard"],
  ["Fri", "Zone 2 bike", "35 min", "Z2 · 65%"], ["Sun", "Steady row", "25 min", "Z2"]];
...
/* ---------- library walls ---------- */
function libraryEx() {
  return `<div class="wall">${EXERCISES.map(([n, m, e, c], i) => `<div class="lc" style="--k:${i}">
    <div class="lc-th"><span class="play"></span></div><div class="lc-n">${n}</div>
    <div class="lc-m">${m} · ${e}</div><div class="lc-t">${c}</div></div>`).join("")}</div>`;
}
function libraryFood() {
  return `<div class="wall">${FOODS.map(([n, k, p, c, f, s], i) => `<div class="lc food" style="--k:${i}">
    <div class="lc-th food-th"></div><div class="lc-n">${n}</div>
    <div class="lc-m">${k} kcal · ${s}</div>
    <div class="lc-macros"><span>P ${p}</span><span>C ${c}</span><span>F ${f}</span></div></div>`).join("")}</div>`;
}

/* ---------- client phone ---------- */
function phone(view = "today") {
  ...
  return `<div class="phone" role="img" aria-label="Forma client app">
    <div class="phone-scr">${today}
      ...
      .map((t, i) => `<span class="${i === 0 ? "on" : ""}">${t}</span>`).join("")}</nav>
    </div></div>`;
}
...
function progressPanel() {
  ...
  return `<div class="app pg"><header class="app-top"><div><div class="app-eye">PROGRESS · AMELIA ROY</div>
    ...
    <div class="pg-grid">
      ...
      <div class="pg-rings">
        ${[["WORKOUT", 88], ["NUTRITION", 92], ["ASSESSMENTS", 100]].map(([l, v]) => `
...
function businessPanel() {
  const k = [["Active clients", "38", "+4"], ["Renewals this week", "6", ""], ["Due this month", "$4,180", ""],
  ["Collected this month", "$11,240", "+8%"], ["Retention", "91%", "+2%"], ["Churn", "4.2%", "−1.1%"],
  ["Subscriptions expiring", "3", ""], ["Avg. client value", "$296", "+$14"]];
  return `<div class="app bz"><header class="app-top"><div><div class="app-eye">BUSINESS</div>
    ...
    <div class="bz-grid">${k.map(([l, v, d]) => `<div class="bz-k"><div class="bz-l">${l}</div>
      <div class="bz-v">${v}</div>${d ? `<div class="bz-d">${d}</div>` : ""}</div>`).join("")}</div>
    <div class="bz-note">Sample values for design review — replace with live figures before launch.</div></div>`;
}

/* ---------- hierarchy labels ---------- */
function hierPanel() {
  const tiers = [["FORMA"], ["Coaches", "Clients", "Templates"],
  ["Workout plans", "Nutrition plans", "Cardio", "Assessments", "Progress", "Messages"],
  ["Subscriptions", "Exercise library", "Food library", "Renewals"]];
  return `<div class="hier">${tiers.map((row, i) => `<div class="hier-r r${i}">
    ${row.map((n) => `<span class="${i === 0 ? "root" : ""}">${n}</span>`).join("")}</div>`).join("")}</div>`;
}

/* ---------- registry ---------- */
export const PANELS = {
  fchat: fragChat(), fsheet: fragSheet(), fpdf: fragPdf(), fphotos: fragPhotos(), fnotes: fragNotes(),
  dash: dashboard(),
  cc0: clientCard(0), cc1: clientCard(4), cc2: clientCard(6),
  bWork: builderWorkout(), bNut: builderNutrition(), bCar: builderCardio(),
  libEx: libraryEx(), libFood: libraryFood(),
  phone: phone(), dashMini: dashboard(),
  prog: progressPanel(), biz: businessPanel(), hier: hierPanel(),
};
...
export function mountUI(container) {
  ...
  return {
    ...
    set(id, { o = 0, x = 0, y = 0, z = 0, rx = 0, ry = 0, s = 1 } = {}) {
      ...
      n.style.transform =
        `translate3d(calc(-50% + ${x}px), calc(-50% + ${y}px), ${z}px) rotateX(${rx}deg) rotateY(${ry}deg) scale(${s})`;
    },
    hideAll() {
      Object.values(nodes).forEach((n) => { n.style.visibility = "hidden"; n.style.opacity = "0"; });
    },
  };
}
