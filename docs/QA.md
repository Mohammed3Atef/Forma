...
> **⚠️ STATUS: PLANNED / NOT YET IMPLEMENTED.** Everything below this notice —
> the `e2e/` suite, its fixtures (`e2e/fixtures/firestore.ts`, `e2e/global-setup.ts`,
> `e2e/fixtures/selectors.ts`), and the `npm run test:e2e*` scripts — describes
> a **target design**, not the current state of the repo. As of this writing:
>
> - The `e2e/` directory does not exist (or is empty) in this codebase.
> - `package.json` has no `test:e2e`, `test:e2e:ui`, `test:e2e:headed`,
>   `test:e2e:report`, or `test:e2e:install` scripts.
> - `docs/QA_REPORT.md` is not generated by anything today.
>
> What **does** exist today is a much smaller headless smoke check:
> `npm run test:smoke` (`scripts/smoke.mjs`), which loads a handful of routes
> in a real browser via `@playwright/test`'s `chromium` launcher and fails on
> any console/page error. It requires `npm run preview` running first. It does
> **not** log in as different roles, assert RBAC, or hit the API directly —
> see `scripts/smoke.mjs` for exactly what it covers.
>
> Treat the rest of this document as the design spec/backlog for the full
> suite described above, to be implemented later — not as a description of
...
## 1. Quick start (planned — not yet implemented, see status banner above)
