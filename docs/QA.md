...
MongoDB Atlas + Vercel API backend**, drives the mobile UI, and asserts the
server-enforced RBAC and data schema directly. The goal is not happy-path
coverage — it is to **break the app and surface everything incomplete before
release.**
...
# 2. Make sure .env has the Mongo/JWT + E2E account vars (see §3)
...
- **Two assertion layers:**
  ...
  2. **API RBAC + schema** via direct calls against the Vercel `/api/*` routes in the
     test process (`e2e/fixtures/firestore.ts`, named for its Firestore-era origin but
     now exercising the Mongo-backed API). This exercises the real security boundary —
     a *denied* request is the expected result for "forbidden" tests.
...
```
# Backend config (also used by the app at build time; see .env.example at repo root)
MONGODB_URI=…
MONGODB_DB=…
JWT_ACCESS_SECRET=…
...
> The four E2E accounts must already exist in the MongoDB database with the
> correct `role` and `accountStatus: active` in their `users` doc. The
...
| Suite | File | Focus |
|---|---|---|
| Preflight | `e2e/preflight.spec.ts` | Env, API/DB reachability, every role logs in + correct role, RBAC allow/deny smoke |
...
| Data integrity | `e2e/data-integrity.spec.ts` | `users`, `coachClients`, `planVersions`, logs location, no legacy `users/{uid}/workoutPlans` |
...
| Coach library | `e2e/coach-library.spec.ts` | Exercise library CRUD via the Library tab, search filter, persistence in `coachExercises` |
...
| Plan versions | `e2e/plan-versions.spec.ts` | Save V1→V2 (active version in `planVersions`), client sees active, coach restores V1, client reverts |
...
- Failures bucketed into: Permission/Security, Broken pages, Static/hardcoded
  data, Mongo schema, Missing/broken features, Console errors, UI/UX.
...
- **Test data accumulates.** Suites that exercise real creation flows (super
  ...
  `…@forma-e2e.test`-style emails each run. Periodically prune the `users` and
  per-client collections for those, or point the suite at a disposable MongoDB
  database.
...
- **Sandbox vs. machine.** The suite must run on a machine that can reach your
  MongoDB Atlas cluster and Vercel API (auth + RBAC) and download the Playwright
  browser. CI or a developer workstation is the right place; an
  offline/locked-down sandbox is not.
...
| API/DB not reachable | Confirm `MONGODB_URI` + `MONGODB_DB` are set and the Vercel dev server / deployed API is reachable. |
